<script setup>
    ...
    const onChangeMode = ({ index, mode }) => {
        state.selected[index].mode = mode
        utils.getWeather(index, state.selected[index].lat, state.selected[index].lon, mode).then(({ labels, temperature }) => {
            state.selected[index].temperature = temperature
            state.selected[index].labels = labels
        })
    }
...
<template>
    <div class="selected">
        <div class="card clear" v-if=" ! _.values(state.selected).length">
            <h3>{{ $t('There are no cities yet') }}</h3>
        </div>
        <div 
            v-else
            class="card"
            v-for="(city, index) in state.selected"
            :key="`city-${index}`">
            <WeatherItem 
                :disabled="true"
                :city="city"
